...
- 데드락
  + 상호 배제
    * 여러 스레드가 한 자원을 공유하나 그 자원은 한 스레드만..
    * 동시에 사용해도 괜찮은 자원 선택(AtomicInteger), 스레드 수 이상으로 자원 수
    , 자원을 점유하기 전에 모든 자원이 있는지 확인
  + 잠금 & 대기
    * 한 자원을 점유하면 끝날때까지 내어 놓지 않음
    * 기아 - 한 스레드가 계속해서 필요한 자원을 점유하지 못한다.
    * 라이브락 - 여러 스레드가 한꺼번에 잠금 단계로 진입하는 바람에 계속해서 자원을 점유했다
    내놓았다 반복
  + 선점 불가
    * 다른 스레드의 자원을 빼앗지 못함함
    * 순서 깨기
