...
- ARGS 클래스 구현
  + enum 으로 상수 처리
  + 각 변수 값에 대한 로직을 함수로 구현 (if-else 에 다 때려박지 않고 분리) -> 수정사항이
  발생시 해당 함수만 수정해주면 됨
  + 테스트 코드를 작성
...
- JUnit 프레임워크
  + 보이스카우트 법칙
    * 코드에 손을 댄다면 더 깨끗하게 만들기 
  + 숨겨진 시간적인 결합
    * 변수에 의존하는 값이 두개이다 보면 디버깅할 때 문제가 생길 수 있음.


<br></br>
## 16. SerialDate 리팩터링
- JCommon 라이브러리에 존재하고 자바 클래스
...
- 주석
  ...
  + 오래되고 엉뚱하고 잘못된 주석을 쓸모없음
  + 중복, 성의없는, 주석처리된 코드
- 환경
  + 여러 단계로 빌드
- 함수
  + 너무 많은 인수, 출력 인수, 플래그 인수, 죽은 함수 (unUsed)
- 일반
  + 한 소스 파일에 여러 언어를 사용, 당연한 동작을 구현하지 않음, 경계를 올바로 처리하지 않는다,
    안전절차 무시, 중복, 추상화 수준이 올바르지 못하다, 기초 클래스가 파생 클라스에 의존한다, 
    과도한 정보, 모듈 인터페이스 크기를 작게, 죽은코드, 수직 분리, 일관성 부족, 잡동사니, 인위적 결합,
    기능 욕심, if-else -> switch/case 문으로, 표준 표기법, 매직 숫자는 명명된 상수로 사용, 
    조건을 캡슐화하기, 부정보다 긍정조건을 사용, 
  + 추이적탐색 > A가 B를 사용하고 B가 C를 사용하더라도 A가 를 알 필요는 없다
- 자바
  + 긴 import 보다는 와일드카드 사용
  + 상수는 상속하지 않는다
  + 상수보다는 enum 권장
- 이름
  + 서술적인 이름 사용
